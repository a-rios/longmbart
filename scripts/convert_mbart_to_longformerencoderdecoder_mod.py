...
# class TrimMBARTConfig(MBartConfig):
...
def create_long_model(
    ...
    # config.architectures = ['TrimMBARTForConditionalGeneration', ]

    # extend position embeddings
    # tokenizer.model_max_length = max_pos
    # tokenizer.init_kwargs['model_max_length'] = max_pos
    # current_max_pos, embed_size = model.model.encoder.embed_positions.weight.shape
    # assert current_max_pos == config.config.max_position_embeddings + 2

    # config.config.max_encoder_position_embeddings = max_pos
    # config.max_decoder_position_embeddings = config.config.max_position_embeddings
